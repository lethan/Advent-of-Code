defmodule Day8 do
  ...
  def run(program, halt \\ :infinity)
  def run({[], _rest, accumlator}, _halt), do: {accumlator, :normal}
  def run({[instruction | rest], previous, accumlator}, halt) do
      ...
      true ->
        case cmd do
          ...
          :jmp ->
            ...
            {comming_commands, previous_commands} = if value > 0 do
  ...
  def possible_programs({[instruction| rest], prev, acc}, front_program, all_programs) do
    ...
    case cmd do
      :jmp ->
        possible_programs({rest, prev, acc}, [instruction | front_program], [{Enum.reverse(front_program) ++ [{{:nop, value}, counter} | rest], prev, acc} | all_programs])
      :nop ->
        possible_programs({rest, prev, acc}, [instruction | front_program], [{Enum.reverse(front_program) ++ [{{:jmp, value}, counter} | rest], prev, acc} | all_programs])
      _ ->
...
program = Day8.import("input_day08.txt")
