defmodule Day14 do
  ...
  defp bitvalues(mask) do
    ...
    |> Enum.reduce({1, 0, 0}, fn value, {multiplier, or_op, and_op} ->
      {multiplier <<< 1, or_op + if value != "1" do 0 else multiplier end, and_op + if value == "0" do 0 else multiplier end}
    end)
  ...
  defp bitvalues2(mask) do
    {_, or_op, and_op, floating} = mask
    |> String.graphemes
    |> Enum.reverse
    |> Enum.reduce({1, 0, 0, []}, fn value, {multiplier, or_op, and_op, floating} ->
      {multiplier <<< 1, or_op + if value == "1" do multiplier else 0 end, and_op + if value != "X" do multiplier else 0 end, if value == "X" do [multiplier | floating] else floating end}
    end)
