  ...
  def print(map) do
    ...
    Enum.each(min_w..max_w, fn w ->
      Enum.each(min_z..max_z, fn z ->
        IO.write("z=#{z}")
        if w_status do IO.write(", w=#{w}") end
        IO.puts("")
        Enum.each(min_y..max_y, fn y ->
          Enum.each(min_x..max_x, fn x ->
            coord = if w_status do
              {x, y, z, w}
            else
              {x, y, z}
            end
            IO.write(if Map.get(map, coord) == :active do "#" else "." end)
          end)
          IO.puts("")
        end)
        IO.puts("")
...
input = Day17.import("input_day17.txt")
