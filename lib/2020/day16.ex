defmodule AoC.Year2020.Day16 do
  def import(file) do
      ...
      cond do
        String.match?(string, ~r/([^:]+): [0-9]+-[0-9]+ or [0-9]+-[0-9]+/) ->
          [key, min1, max1, min2, max2] =
            Regex.run(~r/([^:]+): ([0-9]+)-([0-9]+) or ([0-9]+)-([0-9]+)/, string, capture: :all_but_first)

          {Map.put(
             possible_ranges,
             key,
             {String.to_integer(min1)..String.to_integer(max1), String.to_integer(min2)..String.to_integer(max2)}
...
# input = AoC.Year2020.Day16.import("input/2020/input_day16.txt")

# input
# |> AoC.Year2020.Day16.task1()
# |> IO.puts()

# input
# |> AoC.Year2020.Day16.task2()
# |> IO.puts()
